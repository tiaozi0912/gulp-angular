(function() {
  ...
  var formatter = {
    participants: {
      ...
      label: '# of users',
      count: function(obj, field, count, groups, track) {}
    },
    calls: {
      groups: {
        duration: { values: [10, 30, 60, 300], unit: 'min' },
    ...
    getGroupsData: function(data, field, groups, countFn) {},
    getGroupsDataForFields: function(rawData, groups) {},
    formatData: function(data, dataCategory, dataSubCategory) {},
    distribute: function(data, step, yField) {}
  };
  ...
  /**
   * Deep clone array of JSON objects
   *
  ...
  /**
   * Get group data for each field group
   *
   * @param  {Array} rawData  - raw data, e.x. [{"vendor_id":1,"datetime":1417478400,"delay400":945,"delay800":979},...]
   * @param  {Array} groups  - groups, e.x. [{name:'delay400', label:'delay under 400ms'}, ...]
   * @return {Array} dataset
   */
  formatter.getGroupsDataForFields = function(rawData, groups) {
    var datasets = [],
        data;
   ...
   * @param {string} dataSubCategory - 'duration', 'participants_number'
  ...
  /**
   * Make the data evenly distributes along the xField with specific step
   *
   * @param  {Array} data     - raw data
   * @param  {Number} step    - the step between each xField
   * @param  {Array} yFields  - the names of the y fields
   * @return {String} xField  - the name of the x field
   */
  formatter.distribute = function(data, step, xField, yFields) {
    if (data.length === 0) {
      return data;
    }
    ...
    var start = data[0],
        ...
        steps = parseInt( ( end[xField] - start[xField] ) / step ),
        dataHash = {},
        obj = {},
        originObj,
        newData = [];
    ...
    for (var s = 0; s <= steps; s++) {
      ...
      obj[xField] = start[xField] + s * step;

      originObj = dataHash[obj[xField]];

      if (originObj) {
        newData.push(originObj);
      } else {
